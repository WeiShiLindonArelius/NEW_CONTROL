...
def season_wipe(teams):
    with open('captain_stats', 'w') as capt:
        capt.write('')
    for team in teams:
        team.wins = 0
        team.losses = 0
        ...
        with open('captain_stats', 'a') as capt:
            try:
                capt.write(f"{team.name}: Captain Living for {team.living_captain[0]} of {team.living_captain[0] + team.living_captain[1]} ticks ({100 * round((team.living_captain[0] / (team.living_captain[0] + team.living_captain[1])), 4)}%)\n")
