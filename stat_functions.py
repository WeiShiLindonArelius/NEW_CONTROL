...
def check_equal_season(season1,season2):
    if season1.effect == season2.effect and season1.damage == season2.damage and season1.mitigated == season2.mitigated and season1.player.name == season2.player.name:
        return True
    else:
        return False
...
def region_mvp(season_stats_list, season_count, region, team_names_standings=None):
    #team_names_standings is a dictionary of the team names and the ranks they finished in the season
    full_season_list = season_stats_list[season_count]
    print_all_seasons = True
    ...
    for i in range(3):
        full_season_list[i].print_player_season('region_mvp.txt', team_names_standings[
            full_season_list[i].player.team.replace('*', '').replace('#', '')])


    avg_power = mean([s.player.power for s in full_season_list])
    avg_atk_dmg = mean([s.player.atk_dmg for s in full_season_list])
    avg_atk_spd = mean([s.player.atk_spd for s in full_season_list])
    avg_crit_pct = mean([s.player.crit_pct for s in full_season_list])
    avg_crit_x = mean([s.player.crit_x for s in full_season_list])
    avg_crit_dmg = mean([s.player.crit_dmg for s in full_season_list])
    avg_mit_pct = mean([s.player.mit_pct for s in full_season_list])
    avg_defense_pct = mean([s.player.defense_pct for s in full_season_list])
    avg_defense_abs = mean([s.player.defense_abs for s in full_season_list])
    avg_spawn_time = mean([s.player.spawn_time for s in full_season_list])
    avg_health = mean([s.player.max_health for s in full_season_list])



    with open('playerstats', 'a') as p:
        p.write(f"{region} S{season_count} ALL Player Seasons, Average Stats:\n"
                f"Average Power: {avg_power:.3f}\n"
                f"Average Attack Damage: {avg_atk_dmg:.3f}\n"
                f"Average Attack Speed: {avg_atk_spd:.3f}\n"
                f"Average Critical %: {avg_crit_pct:.3f}\n"
                f"Average Critical Multiplier: {avg_crit_x:.3f}\n"
                f"Average Critical Damage: {avg_crit_dmg:.3f}\n"
                f"Average Mitigated %: {avg_mit_pct:.3f}\n"
                f"Average Defense %: {avg_defense_pct:.3f}\n"
                f"Average Defense Absolute: {avg_defense_abs:.3f}\n"
                f"Average Spawn Time: {avg_spawn_time:.3f}\n"
                f"Average Health: {avg_health:.3f}\n\n")
    for i in range(len(full_season_list)):
        full_season_list[i].print_player_season('playerstats', team_names_standings[full_season_list[i].player.team.replace('*','').replace('#','')])


def best_of_stats(full_season_list, season_count):
    ...
    with open('best_stats', 'a') as p:
        p.write(f"Best Individual Seasons, Season {season_count} (251+ games to qualify for stats): \n")
        grade_seasons(full_season_list, region='All')
...
def series_test(wins, losses):
    ...
    return Fore.RED + f"There is a {round((p_val*100),8)}% chance the result was due to chance alone." + Fore.RESET